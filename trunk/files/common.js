...
/*This method is called to add a word to the Memory list.  (This was before version 0.81).  
It can also be called without arguments to simply re-spell-out memory on the screen.
*/
...
/* This is called from keyPressed(e, code) when SWITCH1 is pressed.  It advances 
highlighting from one letter or row to the next on the virtual keyboard. 
*/
...
/*  This method is called from within other methods to get the last word or word fragment 
before the cursor. 
*/
...
/* This function is called from solveEquationMirror(myText) to eval() individual "words".  
It also expands the function abbreviations if it is a mathematical expression. 
*/
function parse(oele) {
	try {
		var ele = oele;
...
/* This method gets the options values out of the URL.  Note that it doesn't understand 
% encoding.  It is called from JavaScript in the <head /> of each file. 
*/
...
/* First calls solveEquation() to see if it should show a math answer or variable 
expansion.  If solveEquation() returns invalid, it goes on a linear search of frequency 
lists for the top three words to begin with getCurrentWord().  Then, it adds them directly 
to Memory. 
*/
function prediction() {
	var oneSug = "";
	...
	if(oneSug == "") {
		...
		if(currentWord.length > 1) {
			...
			while(!threeSug) {
				...
				if(frequent[jj].toLowerCase() == currentWord.toLowerCase() || oneSug.toLowerCase() == frequent[jj].toLowerCase() || twoSug.toLowerCase() == frequent[jj].toLowerCase()) {
					jj++;
					continue;
				}
...
/* Called ONLY when one-switch-typing is set.  Advances letter after each timerLength 
and re-calls itself by calling setTimeout().
*/
...
/* Locates the cursor and calls solveEquationMirror on everything before the cursor.  
If this returns an error, it calls solveEquationMirror on focused.value
*/
...
function solveEquationMirror(myText) {
	...
	if(Math.abs(parsed) > Math.pow(10, SIG_FIGS)) {
	...
	else if(Math.abs(parsed) < Math.pow(10, -SIG_FIGS) && parsed != 0) {
...
/* Simply unhighlights the proper spot on the virtual keyboard.  If it's in Memory, 
it compresses it to the first mem_lets letters.  (Note it does NOT do this on 
word-completion choices.) */
